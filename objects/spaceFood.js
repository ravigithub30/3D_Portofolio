...
export function addSpaceFood(scene, overrides = {}) {// change for evry new object
  ...
  return new Promise((resolve, reject) => {
    loader.load(
      ...
      (gltf) => {
        ...
        console.log(`[spaceEngine] loaded "${cfg.path}", applied toon shader + black outline to ${meshes.length} mesh(es)`);//change not mandatory
        if (meshes.length === 0) {
          console.warn(`[spaceEngine] "${cfg.path}" loaded but contained NO meshes — check the file itself.`);// change not mandatory
        }
